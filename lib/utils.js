        ...
        if(message&&message.data) {
            try {
                ...
                if(data.selected&&data.myCoords) {
                    ...
                    data.myCoords.forEach(element => {
                        if (element.userName === localStorage.getItem('userName')) {
                            if (_mySquareCount < 20) {
                                ...
                                // console.log(_mySquareCount, element, _myCoords, _markedCoords)
                            } 
                        }
                    ...
                    $('#totalcost').val('$' + _myCoords.length * 100)
                    $('#landpack').html('$' + _myCoords.length * 100)
                    $('#vet').html('$' + _myCoords.length * 100 * 0.9)
                    $('#usdt').html('$' + _myCoords.length * 100)
                    $('#eth').html('$' + _myCoords.length * 100 * 0.8)
                    $('#username').val(localStorage.getItem('userName') ? localStorage.getItem('userName') : '')
                    // Mail account failed
                    if(data.hasOwnProperty('transEmail')) {
                        alert('Admin message does not respond. Please contact with Administrator.')
                    }
                }
